...
document.getElementById("searchPokemon").addEventListener("click", async() => {
    ...
    try {
        ...
        var pokemonMovesOutput = "";
        document.getElementById("moveHeading").innerHTML = `<h2>10 moves ${pokemon.name} can learn</h2>`;
        for (i=0; i < 10; i++) {
            pokemonMovesOutput += `<li>${pokemon.moves[i].move.name}</li>`;
        }
        document.getElementById("pokemonList").innerHTML = pokemonMovesOutput;
    ...
    //4 If  the api returns an error that it can't find the pokemon, display the error to the user and ask them to type a different pokemon name
        ...
        var errorMessage = "";
        errorMessage = `<h3>${pokemonName} is not a valid pokemon name</h3>`;
